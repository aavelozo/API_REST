const { OrdersXItemsController } = require("../registers/orders/OrdersXItemsController");
    ...
    static processPostAsRoute = async(req,res,next,route,arrRoute,level) => {
        try {
            ...
            switch(arrRoute[level].trim().toLowerCase()) {  
                case "get_items_for_production":
                    await OrdersXItemsController.getItemsForProduction(req,res,next);
                    break;                 
                default:                
